package nl.hauntedmc.dungeons.commands;
...
public abstract class Command<T extends Plugin> implements TabExecutor {
   ...
   public boolean onCommand(@NotNull CommandSender sender, @NotNull org.bukkit.command.Command cmd, @NotNull String label, @NotNull String @NotNull [] args) {
      if (this.getPermissionNode() != null && !sender.hasPermission(this.getPermissionNode())) {
         sender.sendMessage(HelperUtils.colorize("&cYou don't have permission to do this!"));
         return true;
      } else if (!this.isConsoleFriendly() && !(sender instanceof Player)) {
         sender.sendMessage(HelperUtils.colorize("&cOnly players can do this!"));
         return true;
      } else {
   ...
   public List<String> onTabComplete(@NotNull CommandSender sender, @NotNull org.bukkit.command.Command cmd, @NotNull String label, @NotNull String @NotNull [] args) {
      if (this.getPermissionNode() != null && !sender.hasPermission(this.getPermissionNode())) {
         return null;
      } else {

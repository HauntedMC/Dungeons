...
public class DungeonRoomContainer {
   ...
   public DungeonRoomContainer(DungeonProcedural dungeon, String namespace, BoundingBox bounds) {
      ...
      this.bounds = bounds;
      this.roomConfig = new AsyncConfiguration(Dungeons.inst());
      ...
      Bukkit.getScheduler().runTask(Dungeons.inst(), () -> this.captureSchematic(dungeon.getEditSession().getInstanceWorld()));
   ...
   public void saveFunctions() {
      ...
      if (Dungeons.inst().isEnabled()) {
         Bukkit.getScheduler().runTaskAsynchronously(Dungeons.inst(), this::saveRoom);
      } else {
         this.saveRoom();
      }
   }
      ...
      if (functions != null) {
         for (DungeonFunction function : functions) {
            ...
            if (!this.bounds.clone().expand(0.0, 0.0, 0.0, 1.0, 1.0, 1.0).contains(loc.getBlockX(), loc.getBlockY(), loc.getBlockZ())) {
               Dungeons.inst().getLogger().warning("WARNING :: Room " + this.namespace + " contains a function out-of-bounds!");
               Dungeons.inst()
                  .getLogger()
                  .info(HelperUtils.colorize("&6-- Function is at X: " + loc.getBlockX() + ", Y: " + loc.getBlockY() + ", Z: " + loc.getBlockZ()));
            } else {
   ...
   public void loadRoom(File file) throws InvalidConfigurationException {
      ...
      Location loc1 = HelperUtils.readLocation(this.roomConfig.getConfigurationSection("Bounds.Corner1"));
      Location loc2 = HelperUtils.readLocation(this.roomConfig.getConfigurationSection("Bounds.Corner2"));
      if (loc1 != null && loc2 != null) {
         ...
         this.spawn = HelperUtils.readLocation(this.roomConfig.getConfigurationSection("Config.SpawnPoint"));
   ...
   public void saveRoom() {
      HelperUtils.writeLocation("Bounds.Corner1", this.roomConfig, new Location(null, this.bounds.getMinX(), this.bounds.getMinY(), this.bounds.getMinZ()));
      HelperUtils.writeLocation("Bounds.Corner2", this.roomConfig, new Location(null, this.bounds.getMaxX(), this.bounds.getMaxY(), this.bounds.getMaxZ()));
      ...
      if (this.spawn != null) {
         HelperUtils.writeLocation("Config.SpawnPoint", this.roomConfig, this.spawn);
      } else {
   ...
   public void captureSchematic(World world) {
      if (this.changedSinceLastSave || !this.schemFile.exists()) {
         if (!this.saving) {
            ...
            if (Dungeons.inst().isEnabled()) {
               Bukkit.getScheduler().runTaskAsynchronously(Dungeons.inst(), write);
            } else {
               write.run();
            }
         }

...
/**
 * Live dungeon map renderer showing local floor geometry and player direction.
 */
public class DungeonMapRenderer extends MapRenderer {
    ...
    /** Creates a renderer bound to one playable instance world. */
    public DungeonMapRenderer(PlayableInstance instance) {
        this.world = instance.getInstanceWorld();
        this.floorDepth = instance.getConfig().getInt("map.floor_depth", 3);
        this.players = new WeakHashMap<>();
    }

    /**
     * Re-renders the map when the player moves to a new block position.
     */
    @Override
        ...
        if (!currentLoc.equals(loc)) {
            ...
            for (int mx = 0; mx <= size; mx++) {
                for (int mz = 0; mz <= size; mz++) {
                    ...
                    for (int scanY = minY; scanY <= maxY; scanY++) {
                        if (!this.world.getBlockAt(x, scanY, z).isEmpty()) {
                            highestY = scanY;
                        }
                    }
            ...
            MapCursor cursor =
                    new MapCursor(
                            (byte) 0,
                            (byte) 0,
                            this.getDirectionId(player.getFacing()),
    ...
    /** Resolves pixel size from map scale. */
    ...
    /** Resolves map cursor direction id from block-facing direction. */

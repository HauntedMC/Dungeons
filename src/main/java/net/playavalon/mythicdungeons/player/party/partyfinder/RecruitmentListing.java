...
public class RecruitmentListing implements Listener {
   ...
   public RecruitmentListing(MythicPlayer host, String label, String description, int partySize, String password) {
      ...
      if (host.hasParty()) {
         this.party = host.getDungeonParty();
      } else if (!MythicDungeons.inst().getPartyPluginName().equalsIgnoreCase("Default")
         && !MythicDungeons.inst().getPartyPluginName().equalsIgnoreCase("DungeonParties")) {
         this.party = new PartyWrapper(host);
      } else {
         MythicDungeons.inst().createParty(host.getPlayer());
         this.party = host.getDungeonParty();
      }
   ...
   public void postListing() {
      ...
      this.broadcaster
         .runTaskTimerAsynchronously(
            MythicDungeons.inst(), 0L, MythicDungeons.inst().getConfig().getInt("General.PartyFinder.ListingBroadcastPeriod", 5) * 1200L
         );
   }
   ...
   public boolean join(MythicPlayer aPlayer) {
   ...
   public void removePlayer(MythicPlayer aPlayer) {
      Player player = aPlayer.getPlayer();
   ...
   public void messagePlayers(String message) {
      this.messagePlayers(message, null);
   }
   ...
   public void setHost(MythicPlayer host) {
      this.host = host;
   }

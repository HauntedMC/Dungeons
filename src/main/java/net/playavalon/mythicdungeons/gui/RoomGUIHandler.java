...
public class RoomGUIHandler {
   ...
   private static void initOpenSlotItem() {
      ...
      meta.setDisplayName(Util.colorize("&7Empty Slot"));
   ...
   public static void initRoomWhitelist(DungeonProcedural dungeon, DungeonRoomContainer room) {
      ...
      Window tableMenu = new Window("whitelist_" + dungeon.getWorldName() + "_" + room.getNamespace(), 54, "&8Edit Room Whitelist");
      ...
      tableMenu.addOpenAction(
         ...
         event -> {
            ...
            if (!room.getRoomBlacklist().isEmpty()) {
               for (WhitelistEntry var15 : room.getRoomBlacklist()) {
                  ;
               }
            }
      ...
      editButton.addLore(Util.colorize("&cNO FUNCTIONALITY HERE!"));
      editButton.addLore(Util.colorize("&cThis button may be replaced with an explainer tooltip."));
      editButton.addAction("click", event -> {
         Player player = (Player)event.getWhoClicked();
         MythicPlayer mPlayer = MythicDungeons.inst().getMythicPlayer(player);
         player.sendMessage(Util.colorize("&cNO FUNCTIONALITY HERE! This button may be replaced with an explainer tooltip."));
      });
      ...
      remButton.addAction("click", event -> {
         ...
         if (mPlayer.isRemovingWhitelistEntry()) {
            ...
            remButton.clearLore();
            remButton.addLore(Util.colorize("&eClick to &aACTIVATE &eremoval mode."));
            tableMenu.updateButtons(player);
         } else {
            mPlayer.setRemovingWhitelistEntry(true);
            remButton.setEnchanted(true);
            remButton.clearLore();
            remButton.addLore(Util.colorize("&eClick to &cDEACTIVATE &eremoval mode."));
   ...
   public static void initConnectorWhitelist() {
      ...
      Window tableMenu = new Window("connector_whitelist", 54, "&8Edit Connector Whitelist");
      tableMenu.setCancelClick(false);
   ...
   private static void updateWeightButton(Button weightButton, DungeonRoomContainer room, Material mat, double weight) {
      ...
      weightButton.addLore(Util.colorize("&eWeight of &6" + weight));
      weightButton.addLore(Util.colorize(""));
      weightButton.addLore(Util.colorize("&7Determines the chance of this room"));
      weightButton.addLore(Util.colorize("&7compared to others. A room with a"));
      weightButton.addLore(Util.colorize("&7weight of 4 is twice as common as a"));
      weightButton.addLore(Util.colorize("&7room with a weight of 2."));
      weightButton.addLore(Util.colorize(""));
      ...
      weightButton.addLore(Util.colorize("&8Click with an item to set icon."));
   }
}

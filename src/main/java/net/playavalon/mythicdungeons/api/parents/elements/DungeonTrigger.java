...
public abstract class DungeonTrigger extends DungeonElement implements Cloneable, ConfigurationSerializable {
   ...
   @Deprecated
   public final void triggerPlayer(MythicPlayer aPlayer) {
      ...
      this.menu.addMenuItem(new ToggleMenuItem() {
         ...
         public void onSelect(Player player) {
            if (!DungeonTrigger.this.limitToRoom) {
               player.sendMessage(Util.colorize(MythicDungeons.debugPrefix + "&aSwitched to '&6trigger must be from the same room&a'"));
            } else {
               player.sendMessage(Util.colorize(MythicDungeons.debugPrefix + "&aSwitched to '&6trigger can come from anywhere&a'"));
            }
   ...
   public void initConditionsMenu() {
      this.conditionsMenu = HotbarMenu.create();
      ...
      this.conditionsMenu.addMenuItem(new MenuItem() {
         ...
         public void onSelect(PlayerEvent event) {
            Player player = event.getPlayer();
            MythicDungeons.inst().getAvnAPI().openGUI(player, "editcondition");
         }
      });
      this.conditionsMenu.addMenuItem(new MenuItem() {
         @Override
         ...
         public void onSelect(PlayerEvent event) {
            Player player = event.getPlayer();
            MythicDungeons.inst().getAvnAPI().openGUI(player, "removecondition");
         }
      });
   }

...
public abstract class Layout implements Cloneable {
   ...
   protected HotbarMenu connectorEditMenu;
   protected HotbarMenu connectorDoorMenu;
   ...
   protected boolean verifyLayout() {
      for (DungeonRoomContainer room : this.dungeon.getUniqueRooms().values()) {
         if (this.countsByRoom.getOrDefault(room, 0).intValue() < room.getOccurrences().getMin()) {
            if (this.DEBUG) {
               MythicDungeons.inst()
                  .getLogger()
                  .warning(
   ...
   public Layout.GenerationResult generate() {
      ...
      if (this.dungeon.getStartRooms().isEmpty()) {
         MythicDungeons.inst().getLogger().warning(Util.colorize("== WARNING :: Your dungeon doesn't have any spawn rooms! =="));
         MythicDungeons.inst().getLogger().warning(Util.colorize("We'll do our best to find a valid spawn point, but it's recommended to define spawn rooms!"));
         MythicDungeons.inst().getLogger().warning(Util.colorize("Any room with a spawn point configured will be considered a valid spawn room."));
      }
      ...
      for (DungeonRoomContainer room : this.dungeon.getUniqueRooms().values()) {
         if (room.getConnectors().isEmpty()) {
            MythicDungeons.inst()
               .getLogger()
               .warning(Util.colorize("== WARNING :: Room '" + room.getNamespace() + "' has no connectors! It won't generate! =="));
         }
      }

      if (this instanceof LayoutBranching && this.config.getConfigurationSection("BranchSettings.TRUNK") == null) {
         MythicDungeons.inst().getLogger().warning("== WARNING :: This dungeon doesn't have a configured 'TRUNK' branch! Default settings will be used. ==");
      }
      ...
      Future<Layout.GenerationResult> future = executor.submit(() -> {
         ...
         do {
            ...
            if (resultx.passed) {
               ...
               MythicDungeons.inst().getLogger().info(Util.colorize("&aGenerated dungeon layout in " + runTime + "ms! Took " + tries + " tries."));
         ...
         MythicDungeons.inst().getLogger().severe(Util.colorize("-- Could not find valid layout after " + tries + " tries... --"));
   ...
   protected Layout.GenerationResult tryLayout() {
      ...
      if (!this.selectFirstRoom()) {
         ...
         if (!this.verifyLayout()) {
            ...
            MythicDungeons.inst().getLogger().info(Util.colorize("&a" + this.totalRooms + " total rooms."));
            return Layout.GenerationResult.SUCCESS;
         }
      }
   }
   ...
   public void initConnectorEditMenu() {
      this.connectorEditMenu = HotbarMenu.create();
      ...
      this.connectorEditMenu.addMenuItem(new MenuItem() {
         ...
         public void onSelect(PlayerEvent event) {
            ...
            if (room != null && connector != null) {
               ...
               player.sendMessage(MythicDungeons.debugPrefix + Util.colorize("&bBuild and configure a custom door for use with functions and mechanics!"));
      ...
      this.connectorEditMenu.addMenuItem(new MenuItem() {
         ...
         public void onSelect(PlayerEvent event) {
            Player player = event.getPlayer();
            MythicDungeons.inst().getAvnAPI().openGUI(player, "connector_whitelist");
         }
      });
      this.connectorEditMenu.addMenuItem(new MenuItem() {
         @Override
         ...
         public void onSelect(PlayerEvent event) {
            ...
            player.sendMessage(MythicDungeons.debugPrefix + Util.colorize("&eConnector copied!"));
         }
      });
      this.connectorEditMenu.addMenuItem(new MenuItem() {
         @Override
         ...
         public void onSelect(PlayerEvent event) {
            ...
            player.sendMessage(MythicDungeons.debugPrefix + Util.colorize("&eConnector cut!"));
         }
      });
      this.connectorEditMenu.addMenuItem(new MenuItem() {
         @Override
                     ...
                     if (dungeon != null && room != null) {
                        ...
                        if (aPlayer.isCopying()) {
                           if (room.getConnector(simpleLoc) != null) {
                              player.sendMessage(MythicDungeons.debugPrefix + Util.colorize("&cThere is already a connector here!"));
                           } else {
                              ...
                              if (!room.applyDirectionAtEdge(simpleLoc)) {
                                 ...
                                 player.sendMessage(MythicDungeons.debugPrefix + Util.colorize("&ePasted connector to the clicked block!"));
                              }
                           }
                        } else {
                           if (aPlayer.isCutting()) {
                              ...
                              player.sendMessage(MythicDungeons.debugPrefix + Util.colorize("&eCut-and-pasted connector to the clicked block!"));
                              aPlayer.setCutting(false);
                           }
                        }
                     }
         ...
         public void onSelect(PlayerEvent event) {
            ...
            if (room != null && connector != null) {
               if ((!connector.getRoomBlacklist().isEmpty() || !connector.getRoomWhitelist().isEmpty()) && !aPlayer.isConfirmRoomAction()) {
                  player.sendMessage(MythicDungeons.debugPrefix + Util.colorize("&eAre you sure you want to delete this connector? Click again to confirm."));
                  aPlayer.setConfirmRoomAction(true);
               } else {
                  aPlayer.setConfirmRoomAction(false);
                  room.removeConnector(connector);
                  player.sendMessage(MythicDungeons.debugPrefix + Util.colorize("&cRemoved connector."));
                  aPlayer.previousHotbar(true);
               }
            }
         }
   ...
   public void initConnectorDoorMenu() {
      this.connectorDoorMenu = HotbarMenu.create();
            ...
            new ChatMenuItem() {
               ...
               public void onSelect(Player player) {
                  ...
                  if (room != null && door != null) {
                     player.sendMessage(Util.colorize(MythicDungeons.debugPrefix + "&eWhat is the name of this door?"));
                     if (door != null) {
                        player.sendMessage(Util.colorize(MythicDungeons.debugPrefix + "&eCurrent name: &6" + door.getNamespace()));
                     }
                  }
               }
               ...
               public void onInput(Player player, String message) {
                  ...
                  if (room != null && connector != null) {
                     ...
                     player.sendMessage(Util.colorize(MythicDungeons.debugPrefix + "&aSet this door's name to '&6" + message + "&a'"));
                     player.sendMessage(
                        Util.colorize(MythicDungeons.debugPrefix + "&bNOTE: You can use this name to open and close the door with functions and mechanics!")
                     );
                  }
               }
            }
            ...
            if (block != null && !block.isEmpty()) {
               ...
               if (room != null && connector != null) {
                  ...
                  if (door.getLocations().contains(target)) {
                     ...
                     player.sendMessage(Util.colorize(MythicDungeons.debugPrefix + "&cRemoved the target block from the list of door blocks."));
                  } else {
                     door.addLocation(target);
                     player.sendMessage(Util.colorize(MythicDungeons.debugPrefix + "&aAdded the target block to the list of door blocks."));
                  }
               }
            }
         }
      ...
      this.connectorDoorMenu.addMenuItem(new ToggleMenuItem() {
         ...
         public void onSelect(Player player) {
            ...
            if (room != null && connector != null) {
               ...
               if (!door.isStartOpen()) {
                  player.sendMessage(Util.colorize(MythicDungeons.debugPrefix + "&aSwitched to '&6Door will start &bOPEN &6(if there's an adjacent room)&a'"));
               } else {
                  player.sendMessage(Util.colorize(MythicDungeons.debugPrefix + "&aSwitched to '&6Door will start &cCLOSED&a'"));
               }
         ...
         public void onSelect(Player player) {
            ...
            if (room != null && connector != null) {
               ...
               if (door.isDisableSound()) {
                  player.sendMessage(Util.colorize(MythicDungeons.debugPrefix + "&aSwitched to '&6Play sound when door opens/closes&a'"));
               } else {
                  player.sendMessage(Util.colorize(MythicDungeons.debugPrefix + "&aSwitched to '&cDO NOT &6play sound when door opens/closes&a'"));
               }
   ...
   public HotbarMenu getConnectorEditMenu() {
      return this.connectorEditMenu;
   }

   public HotbarMenu getConnectorDoorMenu() {
      return this.connectorDoorMenu;
   }
   ...
   public boolean isStrictRooms() {
      return this.strictRooms;
   }
   ...
   public static enum GenerationResult {
      ...
      private GenerationResult(boolean passed) {
         this(passed, "");
      }

      private GenerationResult(boolean passed, String msg) {
         this.passed = passed;
         this.msg = msg;
      }

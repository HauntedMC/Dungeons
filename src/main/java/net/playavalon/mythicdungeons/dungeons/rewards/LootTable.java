...
public class LootTable implements ConfigurationSerializable {
      ...
      this.tableMenu.addOpenAction("load", event -> {
         ...
         for (int slotx = 9; slotx < 54; slotx++) {
            ...
            if (lootItem != null) {
               ...
               buttonx.clearLore();
               buttonx.addLore(Util.colorize("&6" + lootItem.getMinItems() + "-" + lootItem.getMaxItems() + " &eitems"));
               buttonx.addLore(Util.colorize("&eWeight of &6" + lootItem.getWeight()));
               buttonx.addLore(Util.colorize(""));
               buttonx.addLore(Util.colorize("&8Click to edit."));
               buttonx.addLore(Util.colorize("&8Shift-Click to remove."));
            }
         }
         ...
         button.addAction("click", event -> {
            ...
            if (clickType == ClickType.LEFT || clickType == ClickType.RIGHT || clickType == ClickType.SHIFT_LEFT || clickType == ClickType.SHIFT_RIGHT) {
               ...
               if (cursorItem != null && cursorItem.getType() != Material.AIR) {
                  ...
                  button.addLore(Util.colorize("&6" + lootItem.getMinItems() + "-" + lootItem.getMaxItems() + " &eitems"));
                  button.addLore(Util.colorize("&eWeight of &6" + lootItem.getWeight()));
   ...
   private void updateMinButton(Button minButton) {
      ...
      minButton.addLore(Util.colorize("&7Determines the minimum amount of"));
      minButton.addLore(Util.colorize("&7the below items this loot table can"));
      minButton.addLore(Util.colorize("&7generate."));
      minButton.addLore(Util.colorize(""));
   ...
   private void updateMaxButton(Button maxButton) {
      ...
      maxButton.addLore(Util.colorize("&7Determines the maximum amount of"));
      maxButton.addLore(Util.colorize("&7the below items this loot table can"));
      maxButton.addLore(Util.colorize("&7generate."));
      maxButton.addLore(Util.colorize(""));
   ...
   private void updateAllowDuplicatesButton(Button allowDuplicatesButton) {
      ...
      allowDuplicatesButton.addLore(Util.colorize(dupeInfo));
      allowDuplicatesButton.addLore("");
      allowDuplicatesButton.addLore(Util.colorize("&7Toggles whether or not multiple"));
      allowDuplicatesButton.addLore(Util.colorize("&7of the same item can generate in"));
      allowDuplicatesButton.addLore(Util.colorize("&7a loot table."));
   }

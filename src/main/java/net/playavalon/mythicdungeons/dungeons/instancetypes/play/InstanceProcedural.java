...
import net.playavalon.mythicdungeons.api.chunkgenerators.DungeonChunkGenerator;
...
public class InstanceProcedural extends InstancePlayable {
   ...
   public AbstractInstance init() {
      if (this.initialized) {
         ...
         if (!result.isPassed()) {
            MythicDungeons.inst().getLogger().severe(Util.colorize("&c== FAILED TO LOAD DUNGEON!! " + result.getMsg() + " =="));
            return null;
         } else {
   ...
   protected void initMapAndGenerate() {
      ...
      Bukkit.getScheduler()
         .runTask(MythicDungeons.inst(), () -> super.initMap(new DungeonChunkGenerator(this.builder.getClosedConnectorBlock(), this.rooms, blocks)));
   }
   ...
   public void prepValidStartPoint() {
      ...
      if (spawn != null) {
         ...
         spawn = Util.findSafeLocationInBox(this.instanceWorld, firstRoom.getBounds());
      }

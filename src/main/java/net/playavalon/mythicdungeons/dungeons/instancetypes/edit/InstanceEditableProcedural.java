package net.playavalon.mythicdungeons.dungeons.instancetypes.edit;
...
public class InstanceEditableProcedural extends InstanceEditable {
   ...
   public void onLoadGame() {
      ...
      Util.deleteRecursively(roomFolder);
      this.loadFunctions();
      this.functionParticles = new BukkitRunnable() {
         public void run() {
            for (Entry<Location, DungeonFunction> pair : InstanceEditableProcedural.this.functions.entrySet()) {
               ...
               DustOptions dustOptions = new DustOptions(Util.hexToColor(function.getColour()), 1.0F);

               for (MythicPlayer aPlayer : new ArrayList<>(InstanceEditableProcedural.this.players)) {
      ...
      this.roomParticles = new BukkitRunnable() {
         public void run() {
            for (MythicPlayer aPlayer : new ArrayList<>(InstanceEditableProcedural.this.players)) {
               Player player = aPlayer.getPlayer();
               if (aPlayer.getPos1() != null && aPlayer.getPos2() != null) {
                  ParticleUtils.displayBoundingBox(player, Util.captureOffsetBoundingBox(aPlayer.getPos1(), aPlayer.getPos2()));
               }
      ...
      this.roomParticles.runTaskTimerAsynchronously(MythicDungeons.inst(), 0L, 20L);
   ...
   public void saveWorld() {
      new ProcessTimer().run("Saving of " + this.dungeon.getWorldName(), () -> {
         ...
         if (MythicDungeons.inst().isEnabled()) {
            Bukkit.getScheduler().runTaskAsynchronously(MythicDungeons.inst(), () -> this.commitWorld(this.latch));
         } else {
   ...
   public void onCommitWorld() {
      ...
      if (MythicDungeons.inst().isEnabled()) {
         Bukkit.getScheduler().runTask(MythicDungeons.inst(), commit);
      } else {
         commit.run();
      }
   }
   ...
   public void displayRoomParticles(Player player, DungeonRoomContainer room) {
      ...
      boolean useStructureBlock = MythicDungeons.inst().isPaperSupport() && ServerVersion.get().isAfterOrEqual(ServerVersion.v1_20_4);
      if (useStructureBlock
         && !(box.getWidthX() > 48.0)
         && !(box.getHeight() > 48.0)
         && !(box.getWidthZ() > 48.0)

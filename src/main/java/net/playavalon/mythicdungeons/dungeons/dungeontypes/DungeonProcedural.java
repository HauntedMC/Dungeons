...
public class DungeonProcedural extends AbstractDungeon {
   private YamlConfiguration genConfig;
   private YamlConfiguration ruleConfig;
   ...
   public DungeonProcedural(@NotNull File folder, @Nullable YamlConfiguration loadedConfig) throws DungeonInitException {
      ...
      try {
         ...
         if (!configFile.exists()) {
            MythicDungeons.inst().getLogger().info("Creating fresh generator config file for " + folder.getName());
            FileUtils.copyFile(new File(MythicDungeons.inst().getDungeonsFolder(), "default-generator.yml"), configFile);
         }
            ...
            if (inst != null) {
               try {
                  ...
                  if (inst.getStartLoc() == null) {
                     this.timeout(inst, aPlayer);
                     Bukkit.getScheduler().runTask(MythicDungeons.inst(), inst::dispose);
                     return;
                  }
   ...
   public void saveGamerulesFrom(World world) {
      ...
      Runnable task = () -> {
         try {
            ...
            MythicDungeons.inst().getLogger().warning("Failed to save gamerules for dungeon " + this.worldName + "!");
            var2.printStackTrace();
         }
      };
      if (MythicDungeons.inst().isEnabled()) {
         Bukkit.getScheduler().runTaskAsynchronously(MythicDungeons.inst(), task);
      } else {
         task.run();
      }
   }
         ...
         for (File roomFile : files) {
            if (FilenameUtils.getExtension(roomFile.getName()).equals("yml")) {
               try {
                  ...
                  MythicDungeons.inst().getLogger().warning("WARNING :: Could not load dungeon room '" + roomFile.getName().replace(".yml", "") + "'!");
                  MythicDungeons.inst().getLogger().info(Util.colorize("├─ Contains an unsupported element! (Function, trigger, or condition!)"));
                  MythicDungeons.inst().getLogger().info(Util.colorize("├─ You may need to change or delete this function!"));
                  if (var7.getCause() != null) {
                     MythicDungeons.inst().getLogger().info(Util.colorize("&c├─ Error: " + var7.getCause().getMessage()));
                     if (var7.getCause().getMessage().contains("FunctionLootTableRewards")) {
                        MythicDungeons.inst()
                           .getLogger()
                           .info(Util.colorize("&c└─ You can fix this by renaming 'FunctionLootTableRewards' to 'FunctionMythicLootTableRewards'!"));
                     } else {
                        MythicDungeons.inst()
                           .getLogger()
                           .info(Util.colorize("&c└─ This usually happens if the element belonged to another plugin that is no longer present!"));
                     }
                  } else {
                     MythicDungeons.inst().getLogger().info(Util.colorize("&c├─ Error: " + var7.getMessage()));
   ...
   public void addFunction(Location loc, DungeonFunction function) {
      ...
      if (room == null) {
         MythicDungeons.inst().getLogger().warning(Util.colorize("-- An editor tried to add a function outside of a dungeon room!"));
      } else {
